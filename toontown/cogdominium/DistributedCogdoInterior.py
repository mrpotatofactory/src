...
from CogdoExterior import CogdoExterior
...
class DistributedCogdoInterior(DistributedObject.DistributedObject):
    ...
    def __playElevator(self, ts, name, callback):
        ...
        if self.floorModel:
            ...
            if elevIn.isEmpty():
                    elevIn = floorModel.find('**/elevator_in').copyTo(render)
            elevOut = floorModel.find('**/elevator-out').copyTo(render)
            if elevOut.isEmpty():
                    elevOut = floorModel.find('**/elevator_out').copyTo(render)
    ...
    def exitCollectBarrels(self):
        if self._wantBarrelRoom and not self.isBossFloor(self.currentFloor):
            ...
            base.localAvatar.obscureFriendsListButton(0)
            self.waitMusic.stop()
