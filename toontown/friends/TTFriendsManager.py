...
class TTFriendsManager(DistributedObjectGlobal):
    ...
    def setWhisperSCToontaskFrom(self, fromId, *args):
        if not hasattr(base, 'localAvatar'):
            return
        if not hasattr(base.localAvatar, 'setWhisperSCToontaskFrom'):
            return
        base.localAvatar.setWhisperSCToontaskFrom(fromId, *args)
        
    def receiveTalkWhisper(self, fromId, message, mods):
        toon = base.cr.identifyAvatar(fromId)
        if toon:
            base.localAvatar.setTalkWhisper(fromId, 0, toon.getName(), message, mods, 0)
