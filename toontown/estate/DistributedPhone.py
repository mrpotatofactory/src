...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def setupCamera(self, mode):
        ...
        if mode == PhoneGlobals.PHONE_MOVIE_PICKUP:
            camera.lerpPosHpr(4, -4, base.localAvatar.getHeight() - 0.5, 35, -8, 0, 1, other=base.localAvatar, blendType='easeOut')
    ...
    def requestPurchase(self, item, callback, optional = -1, payMethod=0):
        blob = item.getBlob(store=CatalogItem.Customization)
        context = self.getCallbackContext(callback, [item])
        self.sendUpdate('requestPurchaseMessage', [context, blob, optional, payMethod])
