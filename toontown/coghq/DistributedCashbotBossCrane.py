...
class DistributedCashbotBossCrane(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def __enableControlInterface(self):
        ...
        self.closeButton = DirectButton(image=(gui.find('**/CloseBtn_UP'),
         ...
         gui.find('**/CloseBtn_UP')), parent=base.a2dBottomRight, relief=None, scale=2, text=TTLocalizer.CashbotCraneLeave, text_scale=0.04, text_pos=(0, -0.07), text_fg=VBase4(1, 1, 1, 1), pos=(-0.25, 0, 0.175), command=self.__exitCrane)
    ...
    def __exitCrane(self):
        if self.closeButton:
            self.closeButton.destroy()
            self.closeButton = DirectLabel(parent=base.a2dBottomRight, relief=None, text=TTLocalizer.CashbotCraneLeaving, pos=(-0.25, 0, 0.125), text_pos=(0, 0), text_scale=0.06, text_fg=VBase4(1, 1, 1, 1))
        self.__cleanupCraneAdvice()
        self.__cleanupMagnetAdvice()
        self.d_requestFree()
        return
