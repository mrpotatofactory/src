...
if config.GetBool('want-quests-debug', False):
    notify.getDebug = lambda *x: 1
    notify.debug = lambda *x: __import__('sys').stdout.write("%s\n" % x[-1])
...
class CogQuest(LocationBasedQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        return (questCogType == Any or questCogType == cogDict['type']) and avId in cogDict['activeToons'] and self.isLocationMatch(zoneId)
...
class BuildingQuest(CogQuest):
    ...
    def doesBuildingCount(self, avId, track, height, zoneId, avList):
        if not self.isLocationMatch(zoneId):
            return 0
            
        if height < self.getNumFloors():
            return 0

        if track != self.getBuildingTrack():
            if self.getBuildingTrack() != Any:
                return 0
...
class DeliverGagQuest(Quest):
    ...
    def getCompletionStatus(self, av, questDesc, npc = None):
        ...
        if not npcMatches(toNpcId, npc):
            return INCOMPLETE_WRONG_NPC
        
        if npc and av.inventory:
            if av.inventory.numItem(track, level) >= num:
                for i in xrange(num):
...
class VisitQuest(Quest):
    ...
    def getProgressString(self, *args):
        return self.getString()
...
class RecoverItemQuest(LocationBasedQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        if avId in cogDict['activeToons']:
            ...
            holderType = self.getHolderType()
            if holderType == 'level':
                if cogDict['level'] >= holder:
                    return 1
                    
            elif holderType == 'track':
                if cogDict['track'] == holder:
                    return 1
                    
            elif holderType == 'type':
                if cogDict['type'] == holder:
                    return 1
...
class CogdoQuest(LocationBasedQuest):
    trackCodes = ['c', 'l', 'm', 's']
    trackNames = [TTLocalizer.Bossbot, TTLocalizer.Lawbot, TTLocalizer.Cashbot, TTLocalizer.Sellbot]
    ...
    def getObjectiveStrings(self):
        ...
        if count == 1:
            text = TTLocalizer.QuestsCogdoQuestSimple
    ...
    def getSCStrings(self, toNpcId, progress):
        ...
        objective = text % {'type': type}
        location = self.getLocationName()
        return TTLocalizer.QuestsCogdoQuestSCString % {'objective': objective, 'location': location}

    def getHeadlineString(self):
        return TTLocalizer.QuestsCogdoQuestHeadline

    def doesOfficeCount(self, avId, track, zoneId, avList):            
        if not self.isLocationMatch(zoneId):
            return 0
            
        tck = self.getBuildingTrack()
        if tck == Any:
            return 1
...
# boss quests
class BossQuest(CogQuest):        
    def __init__(self, id, quest):
        quest = (Anywhere, quest[0])
        CogQuest.__init__(self, id, quest)
    ...
    def getCogNameString(self):
        numCogs = self.getNumCogs()
        if numCogs == 1:
            return TTLocalizer.ACogBoss
        else:
            return TTLocalizer.CogBosses
    ...
    def doesCogCount(self, *args):
        return 0
        
    def doesBossCount(self, boss):
        if self.__class__ == BossQuest:
            return 1
            
        name = self.__class__.__name__[:-5]
        return boss == name

class VPQuest(BossQuest):
    def getCogNameString(self):
        numCogs = self.getNumCogs()
        if numCogs == 1:
...
QuestDict = {
 ...
 # 18000: BR task for EPP (sellbot)
 # Visit NPC 1
 # Recover 4 items from sellbots (Anywhere / hard)
 # Visit NPC 2
 # Defeat 8 level 7+ COGs in Sellbot HQ
 # Return NPC 1
 # Defeat 5 buildings (Anywhere)
 18000: (BR_TIER + 1, Start,
         (VisitQuest,),
         Any, 3326, NA, 18001,
         TTLocalizer.QuestDialogDict[18000]
         ),
         
 18001: (BR_TIER + 1, Cont,
         (RecoverItemQuest, Anywhere,
          4, 13, Hard, 's', 'track'),
         3326, Same, NA, 18002,
         TTLocalizer.QuestDialogDict[18001]
         ),
         
 18002: (BR_TIER + 1, Cont,
         (VisitQuest,),
         3326, 3133, NA, 18003,
         TTLocalizer.QuestDialogDict[18002]
         ),
         
 18003: (BR_TIER + 1, Cont,
         (CogLevelQuest, ToontownGlobals.SellbotHQ, 8, 7),
         3133, 3326, NA, 18004,
         TTLocalizer.QuestDialogDict[18003]
         ),
         
 18004: (BR_TIER + 1, Cont,
         (BuildingQuest, Anywhere, 5, Any, 1),
         3326, Same, 10003, NA,
         TTLocalizer.QuestDialogDict[18004]
         ),
         
 # 18100: DL task for EPP (cashbot)
 # Visit NPC
 # Defeat 10 level 11+ skelecogs (Cashbot HQ)
 # Defeat 60 cashbots (Anywhere)
 # Defeat 5 3+ cahsbot buildings (Anywhere)
 # Recover 3 pens from Cashbots (Anywhere / difficulty = 5) # HARD AS FUCK ooooo
         
 18100: (DL_TIER + 3, Start,
         (VisitQuest,),
         Any, 9204, NA, 18101,
         TTLocalizer.QuestDialogDict[18100]
         ),
         
 18101: (DL_TIER + 3, Same,
         (SkelecogLevelQuest, ToontownGlobals.CashbotHQ, 10, 11),
         9204, Same, NA, 18102,
         TTLocalizer.QuestDialogDict[18101]
         ),
         
 18102: (DL_TIER + 3, Same,
         (CogTrackQuest, Anywhere, 60, 'm'),
         9204, Same, NA, 18103,
         TTLocalizer.QuestDialogDict[18102]
         ),
         
 18103: (DL_TIER + 3, Same,
         (BuildingQuest, Anywhere, 5, 'm', 3),
         9204, Same, NA, 18104,
         TTLocalizer.QuestDialogDict[18103]
         ),
 
 18104: (DL_TIER + 3, Cont,
         (RecoverItemQuest, Anywhere,
          3, 5005, HardAsFuck, 'm', 'track'),
         9204, Same, 10002, NA,
         TTLocalizer.QuestDialogDict[18104]
         ),
         
 # 1820x: For lawbot and bossbot EPP we have a few options of simple quests
 18200: (PRE_ELDER_TIER, Start,
         (RecoverItemQuest, Anywhere,
          30, 13, Medium, 'mb'),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
          
 18201: (PRE_ELDER_TIER, Start,
         (RecoverItemQuest, Anywhere,
          2, 13, HardAsFuck, 'bf'),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
          
 18202: (PRE_ELDER_TIER, Start,
         (BuildingQuest, Anywhere,
          60, Any, 4),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
          
 18203: (PRE_ELDER_TIER, Start,
         (SkelecogQuest, ToontownGlobals.ToontownCentral,
          1, Any, 4),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
          
 18204: (PRE_ELDER_TIER, Start,
         (SkelecogLevelQuest, Anywhere, 15, 12),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
          
 18205: (PRE_ELDER_TIER, Start,
         (SkeleReviveQuest, Anywhere, 60),
          Any, ToonHQ, Any, NA,
          DefaultDialog),
         
# FF quests
  7000: (FF_TIER, Start,
         (CogQuest, ToontownGlobals.FunnyFarm, 800, Any),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7001: (FF_TIER, Start,
         (CogLevelQuest, Anywhere, 150, 10),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7002: (FF_TIER, Start,
         (CogTrackQuest, Anywhere, 600, 's'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7003: (FF_TIER, Start,
         (CogTrackQuest, Anywhere, 600, 'm'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7004: (FF_TIER, Start,
         (CogTrackQuest, Anywhere, 600, 'l'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7005: (FF_TIER, Start,
         (CogTrackQuest, Anywhere, 600, 'c'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7006: (FF_TIER, Start,
         (SkelecogQuest, Anywhere, 400),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
  ...
  7008: (FF_TIER, Start,
         (BuildingQuest, ToontownGlobals.FunnyFarm, 20, Any, 5),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7009: (FF_TIER, Start,
         (CogdoQuest, Anywhere, 8, Any),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7010: (FF_TIER, Start,
         (CogdoQuest, ToontownGlobals.FunnyFarm, 5, 's'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
  
  #The Love Letter. By: LC (7011 - 7016) 300 Jellybeans
  
  7011: (FF_TIER, Start,
         (DeliverItemQuest, 7015),
         Any, 10006,
         NA, 7012, TTLocalizer.QuestDialogDict[7011]),

  7012: (FF_TIER, Cont,
         (CogQuest, ToontownGlobals.FunnyFarm, 100, Any),
          Same, Same,
          NA, 7013, TTLocalizer.QuestDialogDict[7012]),

  7013: (FF_TIER, Cont,
         (RecoverItemQuest, ToontownGlobals.FunnyFarm, 1, 5005, VeryHard, Any),
         Same, Same,
         NA, (7014, 7015), TTLocalizer.QuestDialogDict[7013]),

  7014: (FF_TIER, Cont,
         (BuildingQuest, ToontownGlobals.FunnyFarm, 50, Any, 3),
         Same, Same,
         NA, 7016, TTLocalizer.QuestDialogDict[7014]),

  7015: (FF_TIER, Cont,
         (CogdoQuest, Anywhere, 50, Any),
         Same, Same,
         NA, 7016, TTLocalizer.QuestDialogDict[7015]),

  7016: (FF_TIER, Cont,
         (DeliverItemQuest, 7016),
         Any, ToonHQ,
         708, NA, TTLocalizer.QuestDialogDict[7016]),

         
  # carry 90 gags (TO DO: CORRECT NPCs)
  17200: (FF_TIER + 1, Start,
         (VisitQuest,),
         Any, 10004,
         NA, 17201, TTLocalizer.QuestDialogDict[17200]),
         
  17201: (FF_TIER + 1, Cont,
         (DeliverItemQuest, 13),
         Any, 9207,
         NA, (17202, 17203), TTLocalizer.QuestDialogDict[17201]),
         
  17202: (FF_TIER + 1, Cont,
         (CogdoQuest, Anywhere, 8, Any),
         Any, 10004,
         NA, 17204, TTLocalizer.QuestDialogDict[17202]),
         
  17203: (FF_TIER + 1, Cont,
         (BuildingQuest, Anywhere, 12, Any, 4),
         Any, 10004,
         NA, 17204, TTLocalizer.QuestDialogDict[17203]),
         
  17204: (FF_TIER + 1, Cont,
         (RecoverItemQuest, ToontownGlobals.FunnyFarm, 20, 13, Medium, Any),
         Any, Same,
         NA, 17205, TTLocalizer.QuestDialogDict[17204]),
         
  17205: (FF_TIER + 1, Cont,
         (DeliverItemQuest, 13),
         Any, 5208,
         NA, 17206, TTLocalizer.QuestDialogDict[17205]),
         
  17206: (FF_TIER + 1, Cont,
         (FactoryQuest, Anywhere, 5),
         Any, Same,
         NA, (17207, 17208), TTLocalizer.QuestDialogDict[17206]),
         
  17207: (FF_TIER + 1, Cont,
         (CFOQuest, 3),
         Any, Same,
         NA, 17209, TTLocalizer.QuestDialogDict[17207]),
  
  17208: (FF_TIER + 1, Cont,
         (CEOQuest, 2),
         Any, Same,
         NA, 17209, TTLocalizer.QuestDialogDict[17208]),
         
  17209: (FF_TIER + 1, Cont,
         (VisitQuest,),
         Any, 10004,
         NA, (17210, 17211), TTLocalizer.QuestDialogDict[17209]),
         
  17210: (FF_TIER + 1, Cont,
         (CogQuest, Anywhere, 50, 'tbc', 'type'),
         Any, Same,
         NA, 17212, TTLocalizer.QuestDialogDict[17210]),
         
  17211: (FF_TIER + 1, Cont,
         (CogQuest, Anywhere, 50, 'mh', 'type'),
         Any, Same,
         NA, 17212, TTLocalizer.QuestDialogDict[17211]),
         
  17212: (FF_TIER + 1, Cont,
         (CogLevelQuest, ToontownGlobals.FunnyFarm, 10, 10),
         Any, Same,
         208, NA, TTLocalizer.QuestDialogDict[17212]),
         
  7300: (FF_TIER + 1, Start,
         (BossQuest, 7),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7301: (FF_TIER + 1, Start,
         (CogQuest, Anywhere, 700, Any),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7302: (FF_TIER + 1, Start,
         (CogdoQuest, Anywhere, 5, 's'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7303: (FF_TIER + 1, Start,
         (CogdoQuest, Anywhere, 5, 'l'),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7304: (FF_TIER + 1, Start,
         (BuildingQuest, ToontownGlobals.FunnyFarm, 30, Any, 1),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
         
  7305: (FF_TIER + 1, Start,
         (FactoryQuest, ToontownGlobals.SellbotHQ, 6),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
  
  7306: (FF_TIER + 1, Start,
         (SkeleReviveQuest, Anywhere, 60),
         Any, ToonHQ,
         Any, NA, DefaultDialog),
}

if config.GetBool('want-cogdo-quests', True):
    # MML (100 jbs)
    # 1. 2 3+ story blgs
    # 2. 3 cogdos
    QuestDict[4225] = (MM_TIER + 1, Start, (BuildingQuest, Anywhere, 4, Any, 3), Any, ToonHQ, NA, 4226, DefaultDialog)
    QuestDict[4226] = (MM_TIER + 1, Cont, (CogdoQuest, Anywhere, 3, Any), Same, ToonHQ, 703, NA, DefaultDialog)
    
    # DG (2 laff points)
    # 2 cogdos
    QuestDict[3159] = (DG_TIER, Start, (CogdoQuest, Anywhere, 2, Any), Any, ToonHQ, 101, NA, DefaultDialog)
    
if config.GetBool('want-elder-boss-quests', True):
    QuestDict[13001] = (ELDER_TIER, Start, (VPQuest, 3), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13002] = (ELDER_TIER, Start, (CFOQuest, 3), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13003] = (ELDER_TIER, Start, (CJQuest, 3), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13004] = (ELDER_TIER, Start, (CJQuest, 2), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13005] = (ELDER_TIER, Start, (CEOQuest, 2), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13006] = (ELDER_TIER, Start, (CEOQuest, 1), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13007] = (ELDER_TIER, Start, (CEOQuest, 3), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13008] = (ELDER_TIER, Start, (BossQuest, 2), Any, ToonHQ, Any, NA, DefaultDialog)
    QuestDict[13009] = (ELDER_TIER, Start, (BossQuest, 3), Any, ToonHQ, Any, NA, DefaultDialog)
    
if config.GetBool('want-top-toons', True):
    def generateQuest(allowList, seed):
        from toontown.uberdog import TopToonsGlobals
        from toontown.suit import SuitDNA
    
        r = random.Random()
        r.seed(seed)
        ct = TopToonsGlobals.CHALLENGE_NONE
        
        questClass = r.choice(allowList)
        
        if questClass == CogQuest:
            ct = TopToonsGlobals.CHALLENGE_COGS
            rewardBase = 100
            type = r.randint(0, len(SuitDNA.suitHeadTypes))
            ...
            if type == 0:
                ...
                index = type
                k = (index % 8) + 1
                type = SuitDNA.suitHeadTypes[index - 1]
                amountRange = (5 * k, 25 * k)
                
            args = [Anywhere, None, type]
            
        elif questClass == CogTrackQuest:
            ct = TopToonsGlobals.CHALLENGE_COGS
            rewardBase = 100
            amountRange = (50, 300)
            track = r.choice(CogTrackQuest.trackCodes)
            args = [Anywhere, None, track]
            
        elif questClass == CogLevelQuest:
            ct = TopToonsGlobals.CHALLENGE_COGS
            rewardBase = 100
            level = r.randint(2, 12)
            amountRange = (500 / level, 600 / level)
            args = [Anywhere, None, level]
            
        elif questClass == BuildingQuest:
            ct = TopToonsGlobals.CHALLENGE_BLDG
            rewardBase = 150
            track = r.choice(CogTrackQuest.trackCodes)
            height = r.randint(1, 5)
            amountRange = (15 / height, 40 / height)
            args = [Anywhere, None, track, height]
        
        elif questClass == CogdoQuest:
            ct = TopToonsGlobals.CHALLENGE_OFFICE
            rewardBase = 200
            amountRange = (5, 15)
            tracks = 's'
            ...
            track = r.choice(tracks)
            args = [Anywhere, None, track]
            
        elif questClass == FactoryQuest:
            ct = TopToonsGlobals.CHALLENGE_FACTORY
            rewardBase = 300
            amountRange = (1, 4)
            args = [Anywhere, None]
            
        elif questClass in (BossQuest, VPQuest, CFOQuest, CJQuest, CEOQuest):
            ct = TopToonsGlobals.CHALLENGE_BOSS
            rewardBase = 550
            k = 1 if questClass == BossQuest else .5
            amountRange = (6 * k, 18 * k)
            args = [None]
            
        amount = r.randint(*amountRange)
        
        maxReward = min(2500, int((rewardBase * amount) / 1.5))
        rewardRange = (rewardBase, max(rewardBase, maxReward))
        reward = r.randint(*rewardRange)
        
        return questClass(0, [x if x is not None else amount for x in args]), ct, reward * 2
...
RewardDict = {100: (MaxHpReward, 1),
 ...
 208: (MaxGagCarryReward, 90, TTLocalizer.QuestsVeryLargeBackpack),
...
# FF tiers:
# 1. 1 laff + 2 laff + carry 300 jbs
# 2. 2 laff + 1 laff + carry 6 toontasks + carry 90 gags
# 3. 1 laff + tp access
 FF_TIER: (100, 101, 708),
 FF_TIER + 1: (101, 100, 504, 208),
 FF_TIER + 2: (100, 306),
}
...
OptionalRewardTrackDict[FF_TIER] = OptionalRewardTrackDict[ELDER_TIER]
OptionalRewardTrackDict[FF_TIER + 1] = OptionalRewardTrackDict[ELDER_TIER]
OptionalRewardTrackDict[FF_TIER + 2] = OptionalRewardTrackDict[ELDER_TIER]
...
from toontown.toon.NPCToons import NPCToonDict
protectionLevel = config.GetFloat('protect-quest-bldgs-level', .45)

r = random.Random(133742069)

for quest in QuestDict.values():
    toNpc = quest[4]
    if toNpc > 1000:
        if r.random() < r:
            x = list(NPCToonDict[toNpc])
            x[-2] = 1
            NPCToonDict[toNpc] = tuple(x)
