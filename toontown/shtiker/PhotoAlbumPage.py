...
class PhotoAlbumPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.scrollList = DirectScrolledList(parent=self, relief=None, forceHeight=0.07, pos=(-0.5, 0, 0), incButton_image=(gui.find('**/FndsLst_ScrollUp'),
         ...
         0.66,
         -0.98,
         0.07), itemFrame_frameColor=(0.85, 0.95, 1, 1), itemFrame_borderWidth=(0.01, 0.01), numItemsVisible=14, items=[])
    ...
    def deleteConfirm(self):
        if os.path.isfile(self.photoPath + self.selectedFileName):        
            ...
            self.deleteCancel()
            self.errorPanel['text'] = TTLocalizer.PhotoPageError1
            self.errorPanel.show()
            self.updateScrollList()
    ...
    def chosePhoto(self, fileName):
        if fileName:
            ...
            if os.path.isfile(self.photoPath + self.selectedFileName):
                ...
                self.errorPanel['text'] = PhotoPageError2
                self.errorPanel.show()
                self.updateScrollList()
        else:
            self.selectedFileName = None
    ...
    def getPhotos(self):
        ...
        for fileName in files:
            if fileName.startswith('toontown-screen') and fileName.endswith('.jpg'):
