...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI, PetBase.PetBase):
        ...
        if not hasattr(self, '_beingCreatedInDB'):
            for i in xrange(len(self.traitList)):
                ...
                if value == 0.0:
                    ...
                    DistributedPetAI.notify.debug("%s: initializing new trait '%s' to %s, seed=%s" % (self.doId,
                     traitName,
                     traitValue,
                     self.traitSeed))
    ...
    def requestDelete(self, task = None):
        DistributedPetAI.notify.debug('PetAI.requestDelete: %s, owner=%s' % (self.doId, self.ownerId))
        if self.hasRequestedDelete():
            DistributedPetAI.notify.debug('PetAI.requestDelete: %s, owner=%s returning immediately' % (self.doId, self.ownerId))
            return
    ...
    def delete(self):
        DistributedPetAI.notify.debug('PetAI.delete: %s, owner=%s' % (self.doId, self.ownerId))
    ...
    def _willDoTrick(self, trickId):
        ...
        DistributedPetAI.notify.debug('_willDoTrick: %s / %s' % (randVal, cutoff)) # .debug
