...
class DistCogdoGameAI(DistributedObjectAI):
    ...
    def gameDone(self, failed=False):
        ...
        if not failed:
            self.sendUpdate('setGameFinish', [globalClockDelta.getRealNetworkTime()])
            taskMgr.doMethodLater(self.finishMovie, self.__finishGame, self.taskName('CogdoFinishMovie'))
            
        else:
            messenger.send(self.gameOverEvent)

    def __finishGame(self, task=None):
