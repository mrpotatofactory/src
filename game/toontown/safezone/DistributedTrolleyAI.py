...
class DistributedTrolleyAI(DistributedObjectAI, FSM):
    ...
    def __activateMinigame(self, task):
        ...
        if players:
            ...
            newbieIds = []
            for avId in players:
                if self.air.questManager.toonRodeTrolley(avId):
                    newbieIds.append(avId)
                
            if config.GetBool('trolley-no-newbies', True):
                # Nametags are fucked for Clerk Clara
                # So no stupid gag intro movie
                newbieIds = []
                
            isTracksDay = datetime.datetime.now().weekday() in (3, 5) # Thursday / Saturday
            doTracks = 0 if isTracksDay else -1
            if len(players) == 1:
                doTracks = -1
            
            mg = createMinigame(self.air, players, self.zoneId, newbieIds=newbieIds, metagameRound=doTracks)
