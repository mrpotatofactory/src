...
class DistributedPicnicTableAI(DistributedNodeAI):
    ...
    def __init__(self, air, zone, tableNumber, x, y, z, h, p, r):
    ...
    def acceptBoarder(self, avId, seatIndex, x, y, z, h, p, r):
        ...
        self.sendUpdate('fillSlot', [avId, seatIndex, x, y, z, h, p, r, globalClockDelta.localToNetworkTime(self.timeOfBoarding), self.doId])
        self.getTableState()
    ...
    def pickGame(self, gameNum):
        x = 0
        for numPlayers in self.seats:
            if numPlayers != None:
                numPlayers += 1
                continue
        if gameNum == 1 and config.GetBool('want-chinese-table', True):
        ...
        elif gameNum == 2 and numPlayers <= 2 and config.GetBool('want-checkers-table', True):
            ...
            self.sendUpdate('setZone', [self.game.zoneId])
        elif gameNum == 3 and numPlayers <= 2 and config.GetBool('want-findfour-table', True):
