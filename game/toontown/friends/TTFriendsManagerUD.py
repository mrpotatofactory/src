...
class TTFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def announceGenerate(self):
        ...
        self.accept('TOON_add_friend', self.__handleToonAddFriend)
        
        # Used for system ban
        # Set access to 300 so devs get notified of such bans
        self.toon2data[1] = {'name': 'GAMESERVER', 'access': 300, 'accId': 1}
        
    def __handleToonAddFriend(self, doId, friendId, flag):
        self.friendsLists.setdefault(friendId, []).append((doId, flag))
        self.friendsLists.setdefault(doId, []).append((friendId, flag))
    ...
    def toonOffline(self, doId):
        ...
        def handleToon(dclass, fields):
            ...
            if doId in self.friendsLists:
                del self.friendsLists[doId]
    ...
    def whisperSCToontaskTo(self, toId, taskId, toNpcId, toonProgress, msgIndex):
        fromId = self.air.getAvatarIdFromSender()
        self.sendUpdateToAvatarId(toId, 'setWhisperSCToontaskFrom', [fromId, taskId, toNpcId, toonProgress, msgIndex])
        
    def sendTalkWhisper(self, toId, message):
        if self.air.chatAgent.detectBadWords(self.air.getMsgSender(), message):
            return
            
        fromId = self.air.getAvatarIdFromSender()
        modifications = self.air.chatAgent.calcModifications(fromId, message)
        self.sendUpdateToAvatarId(toId, 'receiveTalkWhisper', [fromId, message, modifications])
