...
def getPropAppearTrack(prop, parent, posPoints, appearDelay, scaleUpPoint = Point3(1), scaleUpTime = 0.5, startScale = Point3(0.01), poseExtraArgs = None, hpr=None):
...
def doCigarSmoke(attack):
    ...
    dmg = target['hp']
    suitTrack = getSuitTrack(attack)
    cigarPosPoints = [Point3(-0.24, -0.49, -0.38)]
    cigarScaleUpPoint = Point3(5, 5, 5)
    cigarHprPoints = Point3(90, 0, 270)
    cigar = globalPropPool.getProp('cigar')
    cigar.setH(180)
    
    smoke = loader.loadModel('phase_4/models/props/tt_m_efx_ext_smoke')
    smoke.setColor(.5, .5, .5, 1)   
    smokeNode = suit.find('**/joint_head').attachNewNode('smokeNode')
    smokeNode.setH(180)
    R = random.Random(1337)
    for i in xrange(40):
        sn = smoke.copyTo(smokeNode)
        sn.setY(.05 * i)
        sn.setZ(R.randint(-2, 2) / 10.0)
        sn.setX(R.randint(-2, 2) / 10.0)
        sn.setBillboardAxis()
    smokeNode.wrtReparentTo(render)
    smokeNode.hide()
    partTrack = Sequence(Wait(3.5), Func(smokeNode.show),
                         smokeNode.posInterval(3.5, (0, -3, -1.5),
                         (0, 0, 0), other=smokeNode),
                         Func(smokeNode.removeNode)) 
    propTrack = Sequence(getPropAppearTrack(cigar, suit.getRightHand(), cigarPosPoints, 0.8, cigarScaleUpPoint, scaleUpTime=0.5))
    propTrack.append(Wait(4.8))
    propTrack.append(Func(MovieUtil.removeProp, cigar))
    propTrack.append(Func(battle.movie.clearRenderProp, cigar))
    toonTrack = getToonTrack(attack, 5.9, ['cringe'], 4.8, ['sidestep'])
    return Parallel(suitTrack, partTrack, toonTrack, propTrack)
