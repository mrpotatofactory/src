...
class DistributedNPCToon(DistributedNPCToonBase):
    ...
    def setupCamera(self, mode):
        ...
        if mode == NPCToons.QUEST_MOVIE_QUEST_CHOICE or mode == NPCToons.QUEST_MOVIE_TRACK_CHOICE:
            s = camera.posQuatInterval(1.0, (5.0, 9.0, self.getHeight() - 0.5), (155.0, -2.0, 0.0), other=self, blendType='easeOut')
        else:
            s = camera.posQuatInterval(1.0, (-5.0, 9.0, self.getHeight() - 0.5), (-150.0, -2.0, 0.0), other=self, blendType='easeOut')
        s.start()
